...
interface IAdmin {
     ...
     */
    event RemoveWorker(address worker);

    /**
     * Emitted when the admin call the statsInterest
     */
    event StatsInterest(uint256 amount, uint256 total_balance, uint256 address_cnt);
     ...
     */
    function addWorker(address worker) external returns (bool);
     ...
     */
    function removeWorker(address worker) external returns (bool);
     ...
     */
    function statsInterest(uint256 interest) external returns (bool);
     ...
     */
    function changeOwner(address owner) external returns (bool);
     ...
     */
    function AppealRefund(address user, uint256 amount) external;
}
